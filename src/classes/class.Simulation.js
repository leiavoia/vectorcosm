...
export default class Simulation {
	...
	Update( delta ) {
		...
		if ( this.stats.round.time && this.stats.round.time >= this.settings.time ) { 
			...
			const killmes = this.tank.boids.splice(0,numkill);
			killmes.forEach( x=> x.Kill() );
			const num_survivors = this.tank.boids.length;
			...
			if ( diff > 0 ) {
				...
				console.log('parent selection: ' + parent_selection.length )
				for ( let i=0; i < diff; i++ ) {
					let parent = parent_selection[ Math.trunc( Math.random() * parent_selection.length ) ];
					let species = parent ? parent.species : this.settings.species;
					const b = BoidFactory(species, 0, 0, this.tank );
					if ( parent ) {
						console.log('parent was: ' + parent.id );

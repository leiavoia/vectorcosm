...
export default class Sensor {
	...
	Sense() {
		...
		if ( this.type === 'sense' ) {
			...
			let detection = new Array( this.detect?.length || 1 );
			...
			for ( let obj of objs ) {
				...
				const max_dist = (this.r * 2 ) - (this.x + this.y); // not correct math but good enough to cover most situations.
				...
				if ( this.attenuation ) { 
					...
					if ( this.x || this.y ) {
						// TODO: rotate by angle of sensor itself 
						// instead of assuming sensors always point forward
